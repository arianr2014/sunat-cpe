...
public class GraphTypeManager implements TypeResolver
{
    ...
    private Map<String, Class<? extends SunatFrame<?>>> registeredTypes;
    ...
    public Class<? extends SunatFrame> getTypeForDiscriminator(String discriminator)
    {
        return this.getRegisteredTypeMap().get(discriminator);
    }

    public Set<Class<? extends SunatFrame<?>>> getRegisteredTypes()
    {
        return Collections.unmodifiableSet(new HashSet<>(getRegisteredTypeMap().values()));
    }

    private synchronized Map<String, Class<? extends SunatFrame<?>>> getRegisteredTypeMap()
    {
    ...
    private void addTypeToRegistry(Class<? extends SunatFrame<?>> frameType)
    {
    ...
    public void addTypeToElement(Class<? extends SunatFrame<?>> kind, Element element)
    {
        ...
        addProperty(element, SunatFrame.TYPE_PROP, typeValue);
        addSuperclassType(kind, element);
    }
    ...
    {
        return resolve(element, SunatFrame.class);
    }
    ...
    public void init(Element element, Class<?> kind)
    {
        ...
        {
            addTypeToElement((Class<? extends SunatFrame<?>>) kind, element);
        }
    }
    ...
    {
        String typeValue = getTypeValue((Class<? extends SunatFrame>) type);
        return traverser.has(SunatFrame.TYPE_PROP, org.apache.tinkerpop.gremlin.process.traversal.P.eq(typeValue));
    }
    ...
    public <P extends Element, T extends Element> GraphTraversal<P, T> hasNotType(GraphTraversal<P, T> traverser, Class<?> type)
    {
        ...
        return traverser.filter(new Predicate<Traverser<T>>()
        {
            ...
            {
                final Property<String> property = toCheck.get().property(SunatFrame.TYPE_PROP);
                if (!property.isPresent())
                    return true;
