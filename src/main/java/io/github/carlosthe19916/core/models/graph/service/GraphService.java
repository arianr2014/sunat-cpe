...
import io.github.carlosthe19916.core.utils.ExecutionStatistics;
import io.github.carlosthe19916.core.utils.FilteredIterator;
import io.github.carlosthe19916.core.utils.Util;
...
public class GraphService<T extends SunatVertexFrame> implements Service<T> {
    ...
    public static <T extends SunatVertexFrame> T refresh(GraphContext context, T frame) {
        Vertex v = context.getGraph().traversal().V((Long) frame.getId()).next();
        return (T) context.getFramed().frameElement(v, SunatVertexFrame.class);
    }
    ...
    /**
     * Create a new instance of the given {@link SunatVertexFrame} type. The ID is generated by the underlying graph database.
     */
    @Override
    public T create() {
    ...
    @Override
    public T addTypeToModel(final SunatVertexFrame model) {
    ...
    public Iterable<T> findAllByProperty(final String key, final Object value, final boolean filterByType) {
        return ExecutionStatistics.performBenchmarked("GraphService.findAllByProperty(" + key + ")", () -> {
            Class<?> typeSearch = filterByType ? SunatVertexFrame.class : type;
    ...
    public T getUniqueByProperty(String property, Object value, boolean enforceType) throws NonUniqueResultException {
        ...
        T result = null;
        for (SunatVertexFrame item : results) {
            ...
            if (result != null) {
                throw new NonUniqueResultException("Expected unique value, but returned non-unique: " + property + " Conflicting models:"
                        + Util.NL + "\t" + StringUtils.join(item.getClass().getInterfaces(), ", ") + Util.NL + "\t\t" + item.toPrettyString()
                        + Util.NL + "\t" + StringUtils.join(result.getClass().getInterfaces(), ", ") + Util.NL + "\t\t" + result.toPrettyString());
            }
            result = (T) item;
        }
     ...
     */
    public static <T extends SunatVertexFrame> T addTypeToModel(GraphContext graphContext, SunatVertexFrame frame, Class<T> type) {
     ...
     */
    public static <T extends SunatVertexFrame> SunatVertexFrame removeTypeFromModel(GraphContext graphContext, SunatVertexFrame frame, Class<T> type) {
        Vertex vertex = frame.getElement();
        graphContext.getGraphTypeManager().removeTypeFromElement(type, vertex);
        return graphContext.getFramed().frameElement(vertex, SunatVertexFrame.class);
    }
